...
static void exportKtxFaces(Ktx1Bundle& container, uint32_t miplevel, const Cubemap& cm, utils::JobSystem& js);
...
void iblRoughnessPrefilter(
    ...
    Ktx1Bundle container((uint32_t) numLevels, 1, /*true*/false);
    container.info() = {
    ...
    for (ssize_t i = baseExp; i >= ssize_t((baseExp + 1) - numLevels) ; --i) {
        ...
        if (g_type == OutputType::KTX) {
            exportKtxFaces(container, (uint32_t) level, dst, js);
            continue;
        }
...
void extractCubemapFaces(utils::JobSystem& js, const utils::Path& iname, const Cubemap& cm,
    ...
    if (g_type == OutputType::KTX) {
        ...
        Ktx1Bundle container(1, 1, /*true*/false);
        container.info() = {
            .endianness = Ktx1Bundle::ENDIAN_DEFAULT,
            .glType = Ktx1Bundle::R11F_G11F_B10F,
            .glTypeSize = 1,
        ...
        exportKtxFaces(container, 0, cm, js);
...
static void exportKtxFaces(Ktx1Bundle& container, uint32_t miplevel, const Cubemap& cm, utils::JobSystem& js) {
    ...
    //==================---------------------------------
    Image outImage(dim * 2, dim);
    CubemapUtils::cubemapToEquirectangular(js, outImage, cm);
    LinearImage image = toLinearImage(outImage);
    //std::string ext = ImageEncoder::chooseExtension(g_format);
    //std::string filename = ("m" + std::to_string(miplevel) + ext);
    //saveImage(filename, g_format, outImage, g_compression);
    //==================---------------------------------
    for (uint32_t j = 0; j < /*6*/1; j++) {
        ...
        */
        ...
        container.setBlob(blobIndex, uintData.get(), dim * dim * 4 * 2);
    }
}
